...
interface UserGoalJpaRepository extends JpaRepository<UserGoalEntity, Long> {
    ...
    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("SELECT goal FROM UserGoalEntity goal WHERE goal.id = :id AND goal.userId = :userId")
    Optional<UserGoalEntity> findByIdAndUserIdForUpdate(@Param("id") long id, @Param("userId") long userId);
