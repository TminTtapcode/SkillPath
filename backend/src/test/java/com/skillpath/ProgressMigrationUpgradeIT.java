...
class ProgressMigrationUpgradeIT {
    ...
    @Test void upgradesV11ToV13AndCreatesProgressReviewSchema() throws Exception {
        ...
        assertThat(Flyway.configure().dataSource(MYSQL.getJdbcUrl(),MYSQL.getUsername(),MYSQL.getPassword()).locations("classpath:db/migration").load().migrate().targetSchemaVersion).isEqualTo("13");
